...
<script>
	import messageList from '../../../../components/order-notify-list.vue'
	export default {
		data() {
			return {
				flea_id:null,
		...
		methods: {
			...
			async getGoodsComment(flea_id){
				const result = await this.$myRequest({
					url:'/message/getGoodsComments/'+flea_id+'/orders'
				})
			...
			async updateMessageViewStatus(commentId){
				const res = await this.$myRequest({
					url:'/message/updateMessageViewStatus/'+commentId+'/orders',
					method:'PUT'
				})
			},
