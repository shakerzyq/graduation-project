...
public interface BrowseLogMapper {
    ...
    @Select("select count(*) from tb_browse_log where goods_id=#{goodsId} and user_id=#{userId}")
...
/*    @Update("update into tb_browse_log (user_id,goods_id,browse_date,browse_num) values" +
    ...
    @Update("update tb_browse_log set browse_num=browse_num+1 where goods_id=#{goodsId} and user_id=#{userId}")
    Boolean updateBrowseNum(String goodsId, String userId);

    @Insert("insert into tb_browse_log (browse_id,user_id,goods_id,browse_date,browse_num) values (#{browse_id},#{userId},#{goodsId},#{browse_date},0)")
    Boolean insertBrowseLog(String browse_id,String goodsId, String userId,Long browse_date);

    @Update("update tb_browse_log set browse_date=#{currentTimeMillis} where goods_id=#{goodsId} and user_id=#{userId}")
