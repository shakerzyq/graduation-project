...
import zyq.graduation.management.dao.AdminESDataDao;
...
public class AdminUserController {
    ...
    public UserReturn getUncheckedUser(@PathParam("page") Integer page
        ...
        ArrayList<User> users = null;
            users = adminUserMapper.selectUncheckedUser(page-1,limit);
    ...
    public UserReturn getCheckedUsers(@PathParam("page") Integer page
        ...
        System.out.println("page:"+page+"limit"+limit);
        ArrayList<User> users = null;
        users = adminUserMapper.selectCheckedUser(page-1,limit);
//        users = userMapper.selectLikeUncheckedUser("%"+nickname+"%");
    ...
    public UserReturn getUncheckedUserByname(
        ...
        ArrayList<User> users = adminUserMapper.selectLikeUncheckedUser("%"+nickname+"%",page,limit);
    ...
    public UserReturn getCheckedUserByname(
            ...
            ,@PathParam("nickname") String nickname){
        System.out.println("page:"+page+"limit"+limit+"nickname:"+nickname);
        ArrayList<User> users = adminUserMapper.selectLikeCheckedUser("%"+nickname+"%",page,limit);
    ...
    public Boolean disposeRegisterApply(@PathVariable("flea_id") String flea_id
        ...
        if (instruct.equals("1")){
            ...
            return adminUserMapper.updateUserStatus(flea_id);
        }else{
            sendEmailMessage.sendMessage(email,"你的条件不符合跳蚤申请要求");

            return adminUserMapper.deleteUser(flea_id);
        }
    }
    ...
    public User getUserDetail(@PathVariable("flea_id") String flea_id){
        return adminUserMapper.selectUserDetail(flea_id);
    }
    ...
    public boolean updateUserStatus(@PathVariable("order") String order
        ...
        if (order.equals("ban")){
            ...
            if(adminGoodsMapper.updateGoodsFreeze(flea_id,3,0,1)){
                ArrayList<String> goodsList = adminGoodsMapper.selectGoodsByUserId(flea_id,3);
                if (goodsList.size()>0){
                    for(String goodsId:goodsList){
                        esDataDao.updateGoodsStatus("goods",goodsId,"freezing");
                    }
                    //强制删除所有正在交易的订单
                    if(orderMapper.deleteOrder(flea_id)& adminUserMapper.updateUserCredit(flea_id,credit)){
                        return adminUserMapper.updateBanstatus(1,flea_id);
                    }else{
                        return false;
                    }
                }else
            ...
            //商品解除冻结
            adminGoodsMapper.updateGoodsUnFreeze(flea_id,0,3);
            ArrayList<String> goodsList = adminGoodsMapper.selectGoodsByUserId(flea_id,0);
            for(String goodsId:goodsList){
                esDataDao.updateGoodsStatus("goods",goodsId,"living");
            }
            return adminUserMapper.updateBanstatus(0,flea_id);
        }
    ...
    public Boolean temporaryBanUser(@PathVariable("flea_id") String flea_id
        ...
        //所有正在发布的商品
        adminGoodsMapper.updateGoodsFreeze(flea_id,3,0,1);
        adminUserMapper.updateUserCredit(flea_id,credit);
        //强制删除所有正在交易的订单
        orderMapper.deleteOrder(flea_id);
        return adminUserService.temporaryBanUser(flea_id,day);
    }
}
